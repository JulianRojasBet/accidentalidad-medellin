...
Se plantea un modelo de regresión  lineal múltiple que permita predecir el *número de accidentes* usando como predictoras el dia, el mes, el nombre del día, la comúna, el riesgo de accidentalidad del barrio, y un conjunto de variables dummy que determina si es o no un día importante.
```{r}
#Modelo de regresión lineal multiple para el número de accidentes
NumAccidentes_test<-test$`Numero de accidentes`
modelo1<-lm(`Numero de accidentes`~MES+DIA+DIA_NOMBRE+COMUNA+puente_festivo+puente_reyes+semana_santa+feria_flores+puente_semana_santa+festivo_entre_semana+no_festivo_importante+Riesgo, data=train)
...
Podemos ver que la variación de los MSE de prueba y entrenamiento es del 15%, también se ve que tenemos un $R^2=0.4114$ y $R^2_{adj}=0.4108$, por lo tanto el modelo sí explica en un 41% la variabilidad del número de accidentes.
